...
func (h *BaseHandler) ProcessTextResponse(message *tgbotapi.Message) (string, *tgbotapi.InlineKeyboardMarkup, error) {
	// Получаем или создаем чат пользователя
...
func (h *BaseHandler) GetNextQuestion(telegramID int64, title *string) (string, *tgbotapi.InlineKeyboardMarkup) {
	...
	if err != nil {
		...
		default:
			...
			return "Произошла ошибка при получении следующего вопроса", nil
		}
	}
